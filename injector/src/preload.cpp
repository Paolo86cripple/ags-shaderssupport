...
bool EnsureCaptureTexture(int width, int height) {
    if (g_capture_texture && g_capture_width == width && g_capture_height == height) return true;
...
void InitializePipeline() {
    ...
    if (!shader_path || !shader_path[0]) { DebugLog("AGS shader: no shader selected"); return; }
    std::string error;
    if (!g_pipeline.load(shader_path, error)) { DebugLog("AGS shader: failed to load '%s': %s", shader_path, error.c_str()); return; }
...
extern "C" void SDL_GL_SwapWindow(SDL_Window *window) {
    ...
    if (g_in_swap) { g_real_swap(window); return; }
    g_in_swap = true;
    InitializePipeline();
    if (g_pipeline.loaded()) {
        ...
        if(width>0&&height>0&&EnsureCaptureTexture(width,height)) {
            GLint old_read=GL_BACK; glGetIntegerv(GL_READ_BUFFER,&old_read);
