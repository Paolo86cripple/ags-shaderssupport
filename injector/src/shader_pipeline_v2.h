...
private:
    ...
    struct Pass {
        unsigned program = 0;
        int texture=-1, input_size=-1, texture_size=-1, output_size=-1;
        int frame_count=-1, frame_direction=-1, original_size=-1;
        int texel_size=-1, time=-1;
    ...
    std::vector<Pass> _passes; Target _targets[2];
    ...
    static std::string parent_dir(const std::string&);
    static std::string join_path(const std::string&, const std::string&);
    static std::string trim(std::string);
    static std::string lower(std::string);
    static bool parse_bool(const std::string&, bool);
    static float parse_float(const std::string&, float);
    static int parse_int(const std::string&, int);
    static std::string make_stage_source(const std::string&, const char*);
    static bool is_combined_shader(const std::string&);
    static int resolve_dimension(ScaleType, float, int, int);
};
