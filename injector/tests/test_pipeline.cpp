...
int main(int argc, char **argv)
{
    ...
    const bool temporal_history = mode == "history";
    ...
    const unsigned char pixel_a[4]={32,64,128,255}; glTexImage2D(GL_TEXTURE_2D,0,GL_RGBA,1,1,0,GL_RGBA,GL_UNSIGNED_BYTE,pixel_a);
    ...
    if(temporal_history||temporal_feedback){
        const unsigned char pixel_b[4]={10,20,30,255};
        glBindTexture(GL_TEXTURE_2D,source_texture);
        glTexSubImage2D(GL_TEXTURE_2D,0,0,0,1,1,GL_RGBA,GL_UNSIGNED_BYTE,pixel_b);
        pipeline.apply(source_texture,1,1,64,64); glFinish();
    }
    ...
    if(expect_invert||temporal_feedback){expected[0]=223;expected[1]=191;expected[2]=127;expected[3]=255;}
